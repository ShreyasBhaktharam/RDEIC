...
class SpacedSampler:
    def __init__(self, model, schedule="linear", var_type: str="fixed_small"):
        ...
        self.used_num_steps = model.used_timesteps
    ...
    def make_schedule(self, num_steps):
        ...
        # https://github.com/openai/guided-diffusion/blob/main/guided_diffusion/respace.py
        used_timesteps = space_timesteps(self.used_num_steps, str(num_steps))
    ...
    def sample_grad(
        ...
        samples = self.sapced_sampling_grad(
            conditioning, shape, x_T=x_T,
            unconditional_guidance_scale=unconditional_guidance_scale,
            unconditional_conditioning=unconditional_conditioning,
            cond_fn=cond_fn
    ...
    def sapced_sampling_grad(
        ...
        for i, step in enumerate(time_range):
            index = total_steps - i - 1 # t in guided diffusion
            ts = torch.full((b,), step, device=device, dtype=torch.long)
            img = self.p_sample_spaced_grad(img, cond, ts, index=index, unconditional_guidance_scale=unconditional_guidance_scale,
                                        unconditional_conditioning=unconditional_conditioning,
                                        cond_fn=cond_fn)
    ...
    def predict_noise(self, x, t, c, unconditional_guidance_scale, unconditional_conditioning):
        if unconditional_conditioning is None and unconditional_guidance_scale == 1.:
            model_output = self.model.apply_model(x, t, c)
        else:
            model_t = self.model.apply_model(x, t, c)
            model_uncond = self.model.apply_model_unconditional(x, t, c)
